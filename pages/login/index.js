...
import {
    ...
    setLoginStar,
    setLoginIndex,
    setLoginMyMails, setLoginImportant, setLoginSent, setLoginTrash
...
function Login() {
    ...
    const onLogin = () => {
        ...
        if (password === '1234') {
            ...
            dispatch(setLoginMyMails(data.myMails))
            dispatch(setLoginInbox(data.inbox))
            dispatch(setLoginStar(data.star))
            dispatch(setLoginImportant(data.important))
            dispatch(setLoginSent(data.sent))
            dispatch(setLoginTrash(data.trash))
    ...
    return (
        <div className="flex h-screen items-center justify-center">
            ...
            <div className="w-1/3 border-2 flex flex-col items-center pt-5 pb-24 px-6 rounded-md"
                ...
                <div className="flex items-center border-2 px-3 py-1 rounded-full cursor-pointer"
                     onClick={() => router.push('/')}>
                    <div
                        style={{backgroundColor: `${color}`, fontSize: '0.8px'}}
                        className={`rounded-full w-5 h-5 flex items-center justify-center font-semiBold text-white mr-5`}>
                        {nickName}
                    </div>
                ...
                <button onClick={onLogin} className="bg-blue-700 text-white w-20 h-10 rounded-sm self-end mt-10">
                    다음
                </button>
            </div>
        </div>
